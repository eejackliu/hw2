...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    c,c_cache=affine_forward(h,self.params['W2'],self.params['b2'])
    # c=np.exp(c)
    # scores=c/np.sum(c,axis=1,keepdims=True)
    scores=c
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    ############################################################################
    loss,ds=softmax_loss(c,y)
    loss+=self.reg*0.5*np.sum(self.params['W1']**2)+self.reg*0.5*np.sum(self.params['W2']**2)
    dx,dw2,db2=affine_backward(ds,c_cache)
    _,dw1,db1=affine_relu_backward(dx,h_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    ############################################################################
    tmp=hidden_dims
    tmp.insert(0,input_dim)
    tmp.append(num_classes)
    plt_list=[]
    ...
    #   plt_list.append(self.params["W%s"%(num)])
    # for n,i in enumerate(plt_list):
    #   plt.subplot(1,len(plt_list),n+1)
    #   plt.hist(i.flatten(),30,range=[-1,1])
    #   plt.xlabel("layer".join(str(n+1)))
    #   plt.yticks([])
    # plt.show()
    # ############################################################################
    #                             END OF YOUR CODE                             #
  ...
  def loss(self, X, y=None):
    ...
    # of 0.5 to simplify the expression for the gradient.                      #
    ############################################################################
    loss,dx=softmax_loss(final_out,y)
